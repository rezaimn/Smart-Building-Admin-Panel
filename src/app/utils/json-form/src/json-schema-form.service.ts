...
export class JsonSchemaFormService {
	...
	defaultFormOptions: any = {
		...
		defautWidgetOptions: { // Default options for form control widgets
			...
			// enableErrorState: true, // Apply 'has-error' class when field fails validation?               ****** /// json form commented out for a while  ***********
	...
	allComponents: any = [];
	addToComponentList(ctx: any) {
		this.allComponents.push(ctx);
	...
	updateValue(ctx: any, value: any ): void {
		...
		if (this.allComponents) {
			for (let i = 0; i < this.allComponents.length; i++) {
				if (this.allComponents[i].controlName === ctx.nextControl)
					this.allComponents[i].updateQueryParam(value);
